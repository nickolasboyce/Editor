...
public class EFileSaveAs extends JFrame implements ActionListener {
	...
	public String thelines;
	  ...
	  public String[][] therows;
		...
		public String[] mimesrc= { "as", "bat", "cgi", "cmd", "cpp", "cs", "erb", "js", "java", "lua", "php", "pm", "py", "vbs" };
		public String[] mimeexec= { "apk", "app", "bac", "c", "cc", "class", "desktop", "exe", "h", "pl", "pyc", "rb", "sh" };
	...
	public void getSaveAsEvent(String path, String text) {
		...
		String isempty= theWindow.contentArea.getText();
			...
			if (!isempty.equals("")) {
				theEditor.setFileSaveAs(theWindow,filepath,filetext);
			}
	...
	public void actionPerformed(ActionEvent event) {
		...
		if (source==this.cancel)
          	this.dispose();
		if (source==this.ok) {
          	String filepath= getPath(); String filetext= "";
			for (String pattern : openpatterns) {
				if (filepath.matches(pattern)) {  somepath= filepath;
					filetext= theEditor.getCurrentWindow().contentArea.getText(); 
					getSaveAsEvent(filepath,filetext);
				}
			}
		}
		...
		if (source==this.menuButton) {  menu= true;
			somepath= getPath();
			String somefile= table.getValueAt(table.getSelectedRow(),1).toString();
			String filepath= somepath+"/"+somefile;
		...
		if (source==this.pathField) { noadd= false;
			...
			if (somepath.equals("")) { root= true;
				...
				String thetext= theEditor.getCurrentWindow().contentArea.getText();
				getSaveAsEvent(somepath,thetext);
			}
