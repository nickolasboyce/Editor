...
public class EFileSave extends JFrame implements ActionListener {
	...
	public void getSaveEvent(String path, String text) {
		...
		String isempty= theWindow.contentArea.getText();
			...
			if (!isempty.equals("")) {
				theEditor.setFileSave(theWindow,filepath,filetext);
			}
		...
		if (source==this.ok) {
			...
			for (String pattern : openpatterns) {
				if (filepath.matches(pattern)) {  somepath= filepath;
					...
					getSaveEvent(filepath,filetext);
				}
			}
		}
